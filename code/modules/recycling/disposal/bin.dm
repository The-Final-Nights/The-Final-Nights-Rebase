...
/obj/machinery/disposal/proc/trash_living(datum/source, mob/living/shover, mob/living/target, shove_flags, obj/item/weapon)
	...
	target.forceMove(src)
	target.visible_message(span_danger("[GET_GUESTBOOK_NAME(target, shover)] shoves [GET_GUESTBOOK_NAME(target, target)] into \the [src]!"),
		span_userdanger("You're shoved into \the [src] by [GET_GUESTBOOK_NAME(target, shover)]!"), span_hear("You hear aggressive shuffling followed by a loud thud!"), COMBAT_MESSAGE_RANGE, shover)
