...
/datum/element/table_smash/proc/on_interaction(datum/source, mob/user)
	...
	if (!isliving(user.pulling))
		...
		if (user.pulling.loc == source_obj.loc)
			user.visible_message(span_notice("[GET_GUESTBOOK_NAME(user, user)] places [GET_GUESTBOOK_NAME(user, user.pulling)] onto [source_obj]."),
				span_notice("You place [GET_GUESTBOOK_NAME(user, user.pulling)] onto [source_obj]."))
			user.stop_pulling()
	...
	if (pushed_mob.buckled)
		...
		to_chat(user, span_warning("[GET_GUESTBOOK_NAME(user, pushed_mob)] is buckled to [GET_GUESTBOOK_NAME(user, pushed_mob.buckled)]!"))
		return COMPONENT_CANCEL_ATTACK_CHAIN
...
/datum/element/table_smash/proc/perform_table_smash(obj/structure/table/table, mob/living/user)
	...
	if (user.combat_mode)
		...
		pushed_mob.visible_message(span_notice("[GET_GUESTBOOK_NAME(pushed_mob, user)] begins to place [GET_GUESTBOOK_NAME(pushed_mob, pushed_mob)] onto [table]..."), \
							span_userdanger("[GET_GUESTBOOK_NAME(pushed_mob, user)] begins to place [GET_GUESTBOOK_NAME(pushed_mob, pushed_mob)] onto [table]..."))
...
/datum/element/table_smash/proc/riding_offhand_act(mob/living/user, obj/item/riding_offhand/riding_item, obj/structure/table/table)
	...
	carried_mob.visible_message(span_notice("[GET_GUESTBOOK_NAME(carried_mob, user)] begins to[skills_space] place [GET_GUESTBOOK_NAME(carried_mob, carried_mob)] onto [table]..."),
		span_userdanger("[GET_GUESTBOOK_NAME(carried_mob, user)] begins to[skills_space] place [GET_GUESTBOOK_NAME(carried_mob, carried_mob)] onto [table]..."))
...
/datum/element/table_smash/proc/tableplace(mob/living/user, mob/living/pushed_mob, obj/structure/table/table)
	...
	pushed_mob.visible_message(span_notice("[GET_GUESTBOOK_NAME(pushed_mob, user)] places [GET_GUESTBOOK_NAME(pushed_mob, pushed_mob)] onto [table]."), \
		span_notice("[GET_GUESTBOOK_NAME(pushed_mob, user)] places [GET_GUESTBOOK_NAME(pushed_mob, pushed_mob)] onto [table]."))
...
/datum/element/table_smash/proc/tablepush(mob/living/user, mob/living/pushed_mob, obj/structure/table/table)
	...
	pushed_mob.visible_message(span_danger("[GET_GUESTBOOK_NAME(pushed_mob, user)] slams [GET_GUESTBOOK_NAME(pushed_mob, pushed_mob)] onto \the [table]!"), \
		span_userdanger("[GET_GUESTBOOK_NAME(pushed_mob, user)] slams you onto \the [table]!"))
...
/datum/element/table_smash/proc/tablelimbsmash(mob/living/user, mob/living/pushed_mob, obj/structure/table/table)
	...
	pushed_mob.visible_message(span_danger("[GET_GUESTBOOK_NAME(pushed_mob, user)] smashes [GET_GUESTBOOK_NAME(pushed_mob, pushed_mob)]'s [banged_limb.plaintext_zone] against \the [table]!"),
		span_userdanger("[GET_GUESTBOOK_NAME(pushed_mob, user)] smashes your [banged_limb.plaintext_zone] against \the [table]"))
...
/obj/structure/proc/on_disarm_shoved_into(datum/source, mob/living/shover, mob/living/target, shove_flags, obj/item/weapon)
	...
	target.visible_message(span_danger("[GET_GUESTBOOK_NAME(target, shover)] shoves [GET_GUESTBOOK_NAME(target, target)] onto \the [src]!"),
		span_userdanger("You're shoved onto \the [src] by [GET_GUESTBOOK_NAME(target, shover)]!"), span_hear("You hear aggressive shuffling followed by a loud thud!"), COMBAT_MESSAGE_RANGE, shover)
	to_chat(shover, span_danger("You shove [GET_GUESTBOOK_NAME(shover, target)] onto \the [src]!"))
