...
// Fixed the default parameters to use keys that actually exist in the global lists
...
/datum/element/footstep_override/proc/prepare_steps(turf/source, list/steps)
	...
	// Validate that the footstep types exist in their respective global lists before setting them
	// This prevents runtime errors when trying to play non-existent footstep sounds

	// Check if footstep exists in GLOB.footstep
	if(GLOB.footstep[footstep])
		steps[FOOTSTEP_MOB_SHOE] = footstep
	else
		steps[FOOTSTEP_MOB_SHOE] = FOOTSTEP_FLOOR // Safe fallback
		log_runtime("Invalid footstep override: [footstep] not found in GLOB.footstep")

	// Check if barefootstep exists in GLOB.barefootstep
	if(GLOB.barefootstep[barefootstep])
		steps[FOOTSTEP_MOB_BAREFOOT] = barefootstep
	else
		steps[FOOTSTEP_MOB_BAREFOOT] = FOOTSTEP_HARD_BAREFOOT // Safe fallback
		log_runtime("Invalid barefoot override: [barefootstep] not found in GLOB.barefootstep")

	// Check if heavyfootstep exists in GLOB.heavyfootstep
	if(GLOB.heavyfootstep[heavyfootstep])
		steps[FOOTSTEP_MOB_HEAVY] = heavyfootstep
	else
		steps[FOOTSTEP_MOB_HEAVY] = FOOTSTEP_GENERIC_HEAVY // Safe fallback
		log_runtime("Invalid heavy footstep override: [heavyfootstep] not found in GLOB.heavyfootstep")

	// Check if clawfootstep exists in GLOB.clawfootstep
	if(GLOB.clawfootstep[clawfootstep])
		steps[FOOTSTEP_MOB_CLAW] = clawfootstep
	else
		steps[FOOTSTEP_MOB_CLAW] = FOOTSTEP_HARD_CLAW // Safe fallback
		log_runtime("Invalid claw footstep override: [clawfootstep] not found in GLOB.clawfootstep")
