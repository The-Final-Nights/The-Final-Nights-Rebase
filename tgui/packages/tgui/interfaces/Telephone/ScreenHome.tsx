  ...
  return (
    <Stack vertical align="center" justify="center" onClick={onClick}>
      ...
      >
        ...
        {notification !== undefined && notification > 0 && ( //the unread notification thing on the messages app
          <Box
            style={{
              position: 'absolute',
              top: '-1em',
      ...
      {text ? <Stack.Item fontSize={actualSize / 2} style={{ textShadow: '0 0 3px #000, 0 0 3px #000, 0 0 3px #000' }}>{text}</Stack.Item> : null}
    </Stack>
  );
};
...
export const ScreenHome = (props: {
  ...
  if (data.conversations) {
    data.conversations.forEach((conv) => {
      ...
      if (lastSeen === 0 || (conv.last_timestamp && conv.last_timestamp > lastSeen)) {
        unreadCount++;
      }
    });
  }
    ...
    >
      <Stack fill vertical>
        ...
        <Stack align="center" justify="space-between">
          <Stack.Item>
            <Box inline fontFamily="sans-serif" fontSize={4} ml={2} mt={2} style={{ textShadow: '0 0 3px #000, 0 0 3px #000, 0 0 3px #000' }}>
              {time}
            </Box>
            <Box ml={2.5} style={{ textShadow: '0 0 3px #000, 0 0 3px #000, 0 0 3px #000' }}>{date}</Box>
          </Stack.Item>
          <Stack.Item mr={2}>
            <Icon name="cloud" size={2} style={{ filter: 'drop-shadow(0 0 4px #000a)' }} />
          </Stack.Item>
        </Stack>
      </Stack.Item>
      <Stack.Item>
      ...
      <Stack.Item height={1} p={2}>
        <Stack fill align="center" justify="center">
          <Stack.Item p={1}>
            <Icon name="house" color="white" style={{ filter: 'drop-shadow(0 0 4px #000a)' }} />
          </Stack.Item>
          <Stack.Item p={1}>
            <Icon name="circle" color="#ffffffa0" style={{ filter: 'drop-shadow(0 0 4px #000a)' }} />
          </Stack.Item>
          <Stack.Item p={1}>
            <Icon name="circle" color="#ffffffa0" style={{ filter: 'drop-shadow(0 0 4px #000a)' }} />
          </Stack.Item>
