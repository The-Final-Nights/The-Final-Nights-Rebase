	...
	switch(telepathy_type_selected)
		...
		if(TELEPATHY_MIND_READING)
			...
			if(mind_reading_search == "Yes")
				specific_search = tgui_input_text(owner, "What are you trying to mind read from your victim?", "Mind Reading Search Input", max_length = 10240) //TFN EDIT CHANGE - Original : specific_search = tgui_input_text(owner, "What are you trying to mind read from your victim?", "Mind Reading Search Input", max_length = MAX_MESSAGE_LEN)
				if(!specific_search)
...
/datum/discipline_power/auspex/telepathy/proc/sanitize_input_message(input_message)
	//sanitisation!
	input_message = CAN_BYPASS_FILTER(owner) ? strip_html_full(input_message, 10240) : input_message
