...
/datum/emote/living/subtler/run_emote(mob/user, params, type_override, intentional)
	...
	if(SSdbcore.IsConnected() && is_banned_from(user, "Emote"))
		...
		subtler_emote = tgui_input_text(user, "Choose an emote to display.", "Subtler" , max_length = 10240, multiline = TRUE)//TFN EDIT CHANGE - ORIGINAL : subtler_emote = tgui_input_text(user, "Choose an emote to display.", "Subtler" , max_length = MAX_MESSAGE_LEN, multiline = TRUE)
		if(!subtler_emote)
			return FALSE
