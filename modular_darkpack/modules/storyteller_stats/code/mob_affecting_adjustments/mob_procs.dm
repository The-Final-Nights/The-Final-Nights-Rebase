...
/mob/living/proc/st_set_physical_stats(strength_amount, dexterity_amount, stamina_amount, ignore_limits = FALSE)
	if(ignore_limits == TRUE)
		if(strength_amount)
			st_set_stat_ignore_limits(STAT_STRENGTH, strength_amount)
		if(dexterity_amount)
			st_set_stat_ignore_limits(STAT_DEXTERITY, dexterity_amount)
		if(stamina_amount)
			st_set_stat_ignore_limits(STAT_STAMINA, stamina_amount)
	else
		if(strength_amount)
...
/mob/living/proc/st_set_social_stats(charisma_amount, manipulation_amount, appearance_amount, ignore_limits = FALSE)
	if(ignore_limits == TRUE)
		if(charisma_amount)
			st_set_stat_ignore_limits(STAT_CHARISMA, charisma_amount)
		if(manipulation_amount)
			st_set_stat_ignore_limits(STAT_MANIPULATION, manipulation_amount)
		if(appearance_amount)
			st_set_stat_ignore_limits(STAT_APPEARANCE, appearance_amount)
	else
		if(charisma_amount)
...
/mob/living/proc/st_set_mental_stats(perception_amount, intelligence_amount, wits_amount, ignore_limits = FALSE)
	if(ignore_limits == TRUE)
		if(perception_amount)
			st_set_stat_ignore_limits(STAT_PERCEPTION, perception_amount)
		if(intelligence_amount)
			st_set_stat_ignore_limits(STAT_INTELLIGENCE, intelligence_amount)
		if(wits_amount)
			st_set_stat_ignore_limits(STAT_WITS, wits_amount)
	else
		if(perception_amount)
...
/mob/living/proc/st_set_talents_traits(alertness_amount, athletics_amount, awareness_amount, brawl_amount, empathy_amount, expression_amount, intimidation_amount, leadership_amount, streetwise_amount, subterfuge_amount, ignore_limits = FALSE)
	if(ignore_limits == TRUE)
		if(alertness_amount)
			st_set_stat_ignore_limits(STAT_ALERTNESS, alertness_amount)
		if(athletics_amount)
			st_set_stat_ignore_limits(STAT_ATHLETICS, athletics_amount)
		if(awareness_amount)
			st_set_stat_ignore_limits(STAT_AWARENESS, awareness_amount)
		if(brawl_amount)
			st_set_stat_ignore_limits(STAT_BRAWL, brawl_amount)
		if(empathy_amount)
			st_set_stat_ignore_limits(STAT_EMPATHY, empathy_amount)
		if(expression_amount)
			st_set_stat_ignore_limits(STAT_EXPRESSION, expression_amount)
		if(intimidation_amount)
			st_set_stat_ignore_limits(STAT_INTIMIDATION, intimidation_amount)
		if(leadership_amount)
			st_set_stat_ignore_limits(STAT_LEADERSHIP, leadership_amount)
		if(streetwise_amount)
			st_set_stat_ignore_limits(STAT_STREETWISE, streetwise_amount)
		if(subterfuge_amount)
			st_set_stat_ignore_limits(STAT_SUBTERFUGE, subterfuge_amount)
	else
		if(alertness_amount)
...
/mob/living/proc/st_set_skills_traits(animal_ken_amount, crafts_amount, drive_amount, etiquette_amount, firearms_amount, larceny_amount, melee_amount, performance_amount, stealth_amount, survival_amount, ignore_limits = FALSE)
	if(ignore_limits == TRUE)
		if(animal_ken_amount)
			st_set_stat_ignore_limits(STAT_ANIMAL_KEN, animal_ken_amount)
		if(crafts_amount)
			st_set_stat_ignore_limits(STAT_CRAFTS, crafts_amount)
		if(drive_amount)
			st_set_stat_ignore_limits(STAT_DRIVE, drive_amount)
		if(etiquette_amount)
			st_set_stat_ignore_limits(STAT_ETIQUETTE, etiquette_amount)
		if(firearms_amount)
			st_set_stat_ignore_limits(STAT_FIREARMS, firearms_amount)
		if(larceny_amount)
			st_set_stat_ignore_limits(STAT_LARCENY, larceny_amount)
		if(melee_amount)
			st_set_stat_ignore_limits(STAT_MELEE, melee_amount)
		if(performance_amount)
			st_set_stat_ignore_limits(STAT_PERFORMANCE, performance_amount)
		if(stealth_amount)
			st_set_stat_ignore_limits(STAT_STEALTH, stealth_amount)
		if(survival_amount)
			st_set_stat_ignore_limits(STAT_SURVIVAL, survival_amount)
	else
		if(animal_ken_amount)
...
/mob/living/proc/st_set_knowledges_traits(academics_amount, computer_amount, finance_amount, investigation_amount, law_amount, medicine_amount, occult_amount, politics_amount, science_amount, technology_amount, ignore_limits = FALSE)
	if(ignore_limits == TRUE)
		if(academics_amount)
			st_set_stat_ignore_limits(STAT_ACADEMICS, academics_amount)
		if(computer_amount)
			st_set_stat_ignore_limits(STAT_COMPUTER, computer_amount)
		if(finance_amount)
			st_set_stat_ignore_limits(STAT_FINANCE, finance_amount)
		if(investigation_amount)
			st_set_stat_ignore_limits(STAT_INVESTIGATION, investigation_amount)
		if(law_amount)
			st_set_stat_ignore_limits(STAT_LAW, law_amount)
		if(medicine_amount)
			st_set_stat_ignore_limits(STAT_MEDICINE, medicine_amount)
		if(occult_amount)
			st_set_stat_ignore_limits(STAT_OCCULT, occult_amount)
		if(politics_amount)
			st_set_stat_ignore_limits(STAT_POLITICS, politics_amount)
		if(science_amount)
			st_set_stat_ignore_limits(STAT_SCIENCE, science_amount)
		if(technology_amount)
			st_set_stat_ignore_limits(STAT_TECHNOLOGY, technology_amount)
	else
		if(academics_amount)
...
/// Set a specific mob's stat from its stats list, ignoring limits. When you need to create elders/vozhd/similar.
/mob/living/proc/st_set_stat_ignore_limits(stat_path, amount)
	var/datum/st_stat/given_stat = storyteller_stats[stat_path]
	var/score = given_stat?.set_score_ignore_limits(amount)
	update_modifiers_from_stats()
	return score
